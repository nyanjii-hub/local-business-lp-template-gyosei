...
地域密着の行政書士事務所向けランディングページテンプレートです。相続・遺言・会社設立・許認可申請・補助金申請などの相談を受けるためのLPとして使えます。
...
- 画面下部の固定CTA
- 取扱業務、お客様の声、相談事例・サポート内容、事務所情報、Google Mapセクション
- HTML/CSS/JavaScriptのみで動作
- Vercel / Netlify / GitHub Pages にそのまま公開可能

## 使い方

1. `index-gyoseishoshi.html` を開く
2. 以下の項目を実際の事務所情報に差し替える
3. GitHubにアップロードする
...
- 画像はUnsplashの外部画像を使用しています。商用利用時は必要に応じて事務所の写真に差し替えてください。
- お客様の声はサンプルです。実際の声を掲載する場合は、掲載許可や引用ルールを確認してください。
- 電話番号、LINE、Instagram、住所、受付時間は公開前に必ず差し替えてください。
- 「必ず許可が取れる」「補助金が必ず採択される」などの断定表現は避けてください。
- 法律相談そのものではなく、行政書士の業務範囲に合う書類作成・申請手続きのサポート表現に調整してください。
